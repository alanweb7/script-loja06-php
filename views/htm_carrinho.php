...
<head>
	...
	<title>MEU CARRINHO - <?=$data_pagina->meta_titulo?></title>
...
<body>
	...
	<section class="animate-effect" style="margin-top:50px; margin-bottom: 50px;">
		...
		<div class="container">
			...
			<div class="row">
				<div class="col-sm-12">
					<h2 class="titulo_padrao" ><i class="fa fa-shopping-cart" aria-hidden="true"></i> MEU <span>CARRINHO</span></h2>
				...
				<div class="col-sm-12">
					...
					<div class="table-responsive" id="lista_produtos">
						<table class="table tabela_boa" >
							<tr>
								<td style='text-align:center; padding-top:120px; padding-bottom:120px; font-size:22px;' >SEU CARRINHO ESTÁ VAZIO🤔.</td>
							</tr>
						</table>
					</div>
			...
			<?php if($itens_n != 0){ ?>
				...
				<div class="row">
					...
					<div class="col-sm-5">                    
						...
						<div style="padding-top: 8px;"> 
							<div class="input-group">
								<input type="text" name="cupom" id="cupom" class="form-control" placeholder="Digite aqui seu cupom de desconto" value="<?=$data_pedido->cupom?>" >
								<span class="input-group-btn">
									<button class="btn botao_padrao <?=$botao_css?>" type="button" onClick="verifica_cupom()"><i class="fa fa-bolt" aria-hidden="true"></i> INSERIR CUPOM</button>
								</span>
							</div>
						</div>
						...
						<?php if($tipo_envio != 1){ ?>
							...
							<div style="margin-top:30px; margin-bottom:30px;"  >
								...
								<div class="input-group">
									<input type="text" name="cep" id="cep" class="form-control" placeholder="Digite seu CEP" value="<?=$cep_destino?>" onkeypress="Mascara(this,ceppp)" onKeyDown="Mascara(this,ceppp)" size="9" maxlength="9" >
									<span class="input-group-btn">
										<button class="btn botao_padrao <?=$botao_css?>" type="button" onClick="verifica_cep()"><i class="fa fa-search" aria-hidden="true"></i> BUSCAR CEP</button>
									</span>
								</div>
							...
							<div id="lista_frete" >
								<div style='font-size:14px; color:#000000' >* DIGITE SEU CEP PARA CONCLUIR O PEDIDO.</div>
							</div>
						...
						<?php if($tipo_envio == 1){ ?>
							...
							<div style="margin-top:30px; margin-bottom:30px;">
								...
								<div style='font-weight:bold; padding-bottom:5px; padding-top:10px;'>Selecione o balcão de retirada</div>
					...
					<div class="col-sm-7">
						...
						<div class="table-responsive" style="padding-top:5px;" id="lista_valores" >

						</div>
						
						<div id="formas_pagamento" >
							
							<div style='font-weight:bold; text-align:right; padding-top:15px; padding-bottom:5px;'>COMO DESEJA PAGAR?</div>
							
							<?php
							
							$pagseguro_selecionado = false;
							foreach ($pagamento_lista as $key => $value) {
								...
								if($value['id'] <= 10){
									...
									<input type='radio' name='pagamento' id='pagamento_".$value['id']."' value='".$value['id']."' onChange=\"seleciona_pg('".$value['id']."');\" $select style='cursor:pointer;' >
									</div>
									";
									...
									if($_cod_usuario){
										...
										// confere cidade
										if( ($data_dados->estado == $value['estado']) AND ($data_dados->cidade == $value['cidade']) ){
				...
				<div class="row">
					...
					<div class="col-sm-6">
						<div class="ajuste_botoes_carrinho_e" >
							...
							<button type='button' class='botao_padrao <?=$botao_css?>' onClick="window.location='<?=DOMINIO?>';" ><i class="fa fa-arrow-left" aria-hidden="true"></i> CONTINUAR COMPRANDO</button>
					...
					<div class="col-sm-6">
						...
						<div class="ajuste_botoes_carrinho_d" id="div_bt_finalizar" >

							<button type="button" class="btn btn-fefault cart botao_finalizar" onClick="finaliza_pedido();" ><i class="fa fa-arrow-right" aria-hidden="true"></i> FINALIZAR PEDIDO</button>						
						...
						<?php if($carrinho['restricoes'] == 1){ ?>
							...
							<div style="font-size:15px; text-align: right; padding-top: 30px;  ">
								<strong>ATENÇÃO:</strong> Alguns produtos do seu carrinho exigem documentação adicional para conclusao do pedido.
							</div>
	...
	<script>	 
		...
		<?php if($data_balcao){ ?>
			cidades('<?=$data_balcao->uf?>', '<?=$data_balcao->cidade?>');
		<?php } ?>
		...
		<?php if($data_balcao){ ?>
			lista_balcoes('<?=$data_balcao->uf?>', '<?=$data_balcao->cidade?>');
		<?php } ?>
		...
		function remove_carrinho(id){
			$.post('<?=DOMINIO?><?=$controller?>/remover_carrinho/id/'+id, { token:'<?=time()?>' },function(data){
				if(data){
					atualiza_lista_produtos();
				}
			});
		}
		function altera_quantidade(id){
			var quantidade = $('#quantidade_'+id).val();
			$.post('<?=DOMINIO?><?=$controller?>/quantidade', { token:'<?=time()?>', quantidade: quantidade, id: id },function(data){
				if(data){
					atualiza_lista_produtos();
				}
			});
		...
		function verifica_cupom(){
			var cupom = $('#cupom').val();
			$.post('<?=DOMINIO?><?=$controller?>/carrinho_cupom', { token:'<?=time()?>', cupom: cupom },function(data){
				if(data){
					atualiza_valores();
				}
			});
		...
		function seleciona_pg(id){
			$('#div_bt_finalizar').hide();
			$.post('<?=DOMINIO?><?=$controller?>/carrinho_forma_pagamento/id/'+id, { token:'<?=time()?>' },function(data){
				if(data){
					atualiza_valores();
					$('#div_bt_finalizar').show();
				}
			});
		}

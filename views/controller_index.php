...
class index extends controller {
	...
	public function leitura(){
		...
		if(isset($data->codigo)){
			...
			a.botao_".$data->codigo.", .botao_".$data->codigo."{
				...
				background-color:".$data->cor_fundo." !important; 
			...
			a.botao_".$data->codigo.":hover, .botao_".$data->codigo.":hover {
				...
				background-color:".$data->cor_sel_fundo." !important;
				border-radius:".$data->borda_radius."px !important;
	...
	public function cadastro_basico_grv(){
		...
		if($etapa == 3){
			...
			$conexao = new mysql();
			$coisas_confere = $conexao->Executar("select fisica_nome, email from cadastro where codigo='$codigo' AND etapa='4' ");
	...
	public function audios_categoria(){
		...
		if($categoria){
			...
			while($data = $exec->fetch_object()){
				...
				".$data->titulo."
				<div class='audios_descricao' >".$data->previa."</div>
				</div>
		...
		if($i == 0){
			...
			if($data_produto->semestoque == 0){
				...
				if($estoque_total >= 1){
					...
					echo '
					<span>
					<button type="button" class="btn btn-padrao" onClick="submit(\'add_carrinho\')" >
	...
	public function fechar_pedido(){
		...
		if($linha_carrinho != 0){
			...
			//Calcula valores com cupom de desconto
			$valor_desconto_cupom = 0;
			if($data_pedido->cupom){

				$conexao = new mysql();
				$coisas_cupom = $conexao->Executar("SELECT * FROM cupom_lista where cupom='$data_pedido->cupom' ");
				$data_cupom = $coisas_cupom->fetch_object();
				...
				if(isset($data_cupom->codigo)){
					...
					if($valor_subtotal > $data_promo->valor_minimo){
						...
						if($data_pedido->cupom_desconto_porc > 0){

							$percentual = $data_pedido->cupom_desconto_porc / 100.0;
							$valor_desconto_cupom = $valores->trata_valor_calculo( $percentual * $valor_subtotal );
							$valor_desconto_cupom = $data_pedido->cupom_desconto_fixo + $valor_desconto_cupom;

						} else {
							$valor_desconto_cupom = $data_pedido->cupom_desconto_fixo;
						}
			...
			if($data_pedido->forma_pagamento){
				...
				if($data_pedido->forma_pagamento_desc_porc > 0){

					$percentual = $data_pedido->forma_pagamento_desc_porc / 100.0;
					$valor_desconto_forma_pag = $valores->trata_valor_calculo( $percentual * $valor_subtotal );
					$valor_desconto_forma_pag = $data_pedido->forma_pagamento_desc_fixo + $valor_desconto_forma_pag;

				} else {
					$valor_desconto_forma_pag = $data_pedido->forma_pagamento_desc_fixo;
				}
